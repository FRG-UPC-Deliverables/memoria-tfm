...
interfaces are \textit{EnCase Forensic} and \textit{Autopsy}. But there are no
open-source user-friendly interfaces that can run over \textit{Windows}, 
\textit{Linux} and \textit{Max OS}.

Those softwares offer the possibility to see the file system of an image
...
cases. In this project a new software, \textit{Img-spy}, was created to perform
those operations.

To do so, \textit{The Sleuth Kit} C library was used to build a wrapper called
\textit{The Sleuth Kit JavaScript}. This wrapper allows JavaScript to perform a
digital forensics analysis.

But for an application, is not enough have useful functionalities, so it
must be comfortable for the user. This obvious purpose is very difficult to
...
This software is also a good example of how a scalable application can be
build using flux-like architecture based on \textit{React-Redux} packages.
Those modern libraries create a unidirectional data flow that starts with
actions that modifies the current state of the application and it affects to
the view of the user interface.
\end{overview}
